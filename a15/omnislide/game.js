...
PS.init = function( system, options ) {
    ...
    PS.audioLoad("WallSlide1", {path: "audiofiles/", fileTypes: ["mp3"]});
    PS.audioLoad("WallSlide2", {path: "audiofiles/", fileTypes: ["mp3"]});
    PS.audioLoad("WallSlide3", {path: "audiofiles/", fileTypes: ["mp3"]});
    PS.audioLoad("WallSlide4", {path: "audiofiles/", fileTypes: ["mp3"]});

    PS.audioLoad("WThud1", {path: "audiofiles/", fileTypes: ["mp3"]});
    PS.audioLoad("WThud2", {path: "audiofiles/", fileTypes: ["mp3"]});
    PS.audioLoad("WThud3", {path: "audiofiles/", fileTypes: ["mp3"]});
    PS.audioLoad("WThud4", {path: "audiofiles/", fileTypes: ["mp3"]});

    PS.audioLoad("StringsBGM", {path: "audiofiles/", fileTypes: ["mp3"]});
    PS.audioLoad("SynthBGM", {path: "audiofiles/", fileTypes: ["mp3"]});
...
PS.exitGrid = function( options ) {
    if(game_active && held !== null && cursor_bead[0] === held[0] && cursor_bead[1] === held[1]) {
        PS.scale(held[0], held[1], 65);
        bad_positions.push(held);
        PS.audioPlay("WThud" + PS.random(4), {path: "audiofiles/", fileTypes: ["mp3"], volume: 0.2});
...
function loadLevel(){
    ...
    PS.glyphColor(PS.ALL, PS.ALL, bg_color);
    ...
    fade_thread = PS.timerStart(50, loadLevelThread);
}

function loadLevelThread(){
    const pal = level_palettes[current_level];
    const pal_dat = pal.data;
    const grid = level_grids[current_level];
    const grid_dat = grid.data;
    ...
    PS.gridColor(wall_color);

    PS.timerStop(fade_thread);
}

function startLevel(){
    toggleFade(false);

    PS.glyphFade(player_pos[0], player_pos[1], 20);

    PS.gridColor(getWallColor(level_palettes[current_level].data));
...
function beatLevel(){
    PS.audioPlay("fx_powerup7", {volume: 0.1});
...
function moveEntity(x, y, vec, check_others = false){
    ...
    for(let i = 0; i < check.length; i++){
        ...
        if(x_o >= grid_x || y_o >= grid_y || x_o < 0 || y_o < 0 || (PS.data(x_o, y_o) === 1 && PS.color(x_o, y_o) !== my_color)){
            ...
            //PS.debug(grid_x + "\n")
            //PS.debug(grid_y + "\n")
            bad_positions.push(check[i]);
        }
    }

    for(let bad_pos of bad_positions){
        PS.scale(bad_pos[0], bad_pos[1], 65);
        held = null;
        held_color = null;
        game_active = false;
    }
    ...
    PS.audioPlay("WallSlide" + PS.random(4), {path: "audiofiles/", fileTypes: ["mp3"], volume: 0.35});
...
function mainLoop() {
    for(let bad_pos of bad_positions){
        if(bad_pos[0] === player_pos[0] && bad_pos[1] === player_pos[1]){
            PS.scale(bad_pos[0], bad_pos[1], 73);
        }
        else{
            PS.scale(bad_pos[0], bad_pos[1], 100);
        }
        game_active = true;
    }
...
function playStrings(){
    PS.audioPlay("StringsBGM", {path: "audiofiles/", fileTypes: ["mp3"], volume: 0.15, onEnd: playSynth});
}

function playSynth(){
    PS.audioPlay("SynthBGM", {path: "audiofiles/", fileTypes: ["mp3"], volume: 0.15, onEnd: playStrings});
}